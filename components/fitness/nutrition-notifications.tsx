...
import { BellRing, CheckCircle2, ShoppingCart, Waves, Apple, AlertTriangle } from "lucide-react";

import { useFitness } from "@/components/providers/fitness-provider";

export function NutritionNotifications({ onAction }: { onAction?: (route: string) => void }) {
  const { dailyLog, calorieTargets, macroTargets } = useFitness();
  const [notifications, setNotifications] = useState<any[]>([]);
  const [unreadCount, setUnreadCount] = useState(0);

  useEffect(() => {
    if (!dailyLog || !calorieTargets || !macroTargets) return;
    const url = `/api/nutrition/notifications?calories=${dailyLog.caloriesConsumed}&protein=${dailyLog.proteinConsumed}&targetCal=${calorieTargets.activeTarget}&targetPro=${macroTargets.protein.grams}&workout=${dailyLog.workoutCompleted ? "true" : "false"}`;
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
      ...
      .catch(() => {});
  }, [dailyLog, calorieTargets, macroTargets]);
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case "info": return <ShoppingCart className="h-4 w-4 text-[var(--accent)]" />;
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {n.actionLabel && (
                      <button
                        onClick={() => { markAsRead(n.id); onAction?.(n.actionRoute || "#"); }}
